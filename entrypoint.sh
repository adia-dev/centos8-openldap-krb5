#!/bin/bash

setup_services() {
    sleep 10
    echo "Waiting for systemd to launch..."
    /srv/openldap/scripts/setup_firewalld.sh
    /srv/openldap/scripts/setup_rsyslog.sh
    /srv/openldap/scripts/setup_slapd.sh


    tail -f /var/log/ldap.log &

    telegraf --config /srv/telegraf/openldap.conf
}



# Function to perform health check
perform_health_check() {
    local max_retries=25
    local retries=1
    local retries_delay=1
    until ldapsearch -x -b "" -s base "objectclass=*" -H ldapi:/// >/dev/null 2>&1; do
        if [ $retries -lt $max_retries ]; then
            echo "Waiting for slapd to start, retrying in $retries_delay seconds (Retry $retries of $max_retries)..."
            ((retries++))
            sleep $retries_delay
        else
            echo "Error: slapd failed to start after $max_retries retries."
            exit 1
        fi
    done
}

# Main execution
setup_services &
perform_health_check &

# Start the init process
echo "Starting the init process..."
exec /usr/sbin/init
echo "Init process completed, systemd operations should be accessible now."
